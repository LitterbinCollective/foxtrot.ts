...
export default class TalkingBenVoiceModule extends BaseModule {
  ...
  private startEvent() {
    ...
    const playerKillEvent = () => {
      debug(
        'playerKill event received, assuming that it will play the next sound'
      );
      this.playSound('ben', 'Ben?');
      this.voice.once('playerKill', () => this.startListening());
    };
  ...
  private respond() {
    ...
    this.stopListening();
    const responses = ['disgust', 'laugh', 'no', 'yes'];
    ...
    this.voice.once('playerKill', () => !this.kill && this.startListening());
  }
  ...
  private startListening() {
    ...
    if (!this.mixer || !this.sox) {
      ...
      this.sox.stdout.on(
        'error',
        (error: any) => error.code !== 'EPIPE' && console.error(error.code)
      );
      this.sox.stdin.on(
        'error',
        (error: any) => error.code !== 'EPIPE' && console.error(error.code)
      ...
      this.sox.stdout.on('data', (chunk: Buffer) => {
        ...
        for (let v = 0; v < chunk.length / SAMPLE_BYTE_LEN; v++) {
          ...
          if (Math.abs(samples) / 32678 >= this.MIN_VOLUME) silent = false;
        }
        ...
        if (silent) {
          ...
          if (this.heard) {
            ...
            this.MAX_WAIT_TIME_SILENCE
          ) {
            this.startedAt = Date.now();
            this.destroy();
            this.voice.logChannel.createMessage(
              '*[Ben has hang up the phone.]*'
            );
          }
        } else
