...
export default class GoogleAssistantVoiceModule extends EventEmitter {
  ...
  private receivePacket ({
    ...
    if (this.decoders[userId]) { this.decoders[userId].write(data) }
  }
  ...
  private async onTranscription ({
    ...
    if (!done) {
      ...
      if (this.transcriptMessage) {
        ...
        if (typeof this.transcriptMessage !== 'boolean') { await getRidOfTranscriptMessage() } else {
          this.once(
      ...
      if (this.responseMessage) {
        ...
        if (typeof this.responseMessage !== 'boolean') { await editResponseMessage() } else {
          this.once(
  ...
  private setupConversation (conv: any) {
    ...
    conv
      ...
      .once('ended', (error: Error, continueConversation: boolean) => {
        if (error) { this.onError(error, true) } else if (continueConversation) {
      ...
      .on('error', async (error: Error) =>
        await this.onError(error, false)
      )
  ...
  public async startListening () {
    ...
    if (this.sox) { this.getRidOfVoiceReceiver() }
