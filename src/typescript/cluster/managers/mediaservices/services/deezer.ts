...
export default class DeezerService extends MediaService {
  ...
  private async getAuthTokens() {
    ...
    try {
      ...
      const userData: DeezerGetUserDataResponse = await res.json();
      ...
      const apiToken = await this.fetchAPI<DeezerAPIToken>(
        'post',
  ...
  private async getAlbumTracks(albumId: string) {
    const albumData = await this.fetchAPI<DeezerAlbumData>(
      'get',
  ...
  private async createStream(trackToken: string, songId: string) {
    ...
    const data: DeezerGetUrlResponse = await res.json();
