...
import { Functions, UserError } from '@clu/utils';
import { Constants } from '@sha/utils';
...
export class BaseCommand extends Command.Command {
    ...
    */
    try {
      ...
      const manageGuildCheck = this.manageGuildOnly
        ? Functions.checkPermission(ctx, DetritusConstants.Permissions.ADMINISTRATOR) ||
          Functions.checkPermission(ctx, DetritusConstants.Permissions.MANAGE_GUILD) ||
  ...
  public async onRunError(
    ...
    if (ctx.channel?.canMessage) {
      ...
      const embed = await Functions.buildRuntimeErrorEmbed(ctx.guild, error);
      ctx.reply({ embed });
  ...
  public async onTypeError(
    ...
    if (!ctx.guild) return;
    if (ctx.channel?.canMessage) {
      const embed = await Functions.buildArgumentErrorEmbed(ctx.guild, errors);
      ctx.reply({ embed });
    } else if (ctx.channel?.canAddReactions)
